layui.define(['jquery', 'laytpl', 'layer'], function (e) {
  ...
  var hint = layui.hint(),
    ...
    system = {
      config: {
        template: ['<div class="layui-form-autocomplete">', '<dl class="layui-anim layui-anim-upbit"><table class="layui-table"><thead></thead><tbody>', '</tbody></table></dl>', '</div>'].join(''),
        layout: [' <tr><dd data-index="{{d.index}}">{{d.text}}</dd></tr>'].join(''),
        template_head: '{{d.head}}',
        template_txt: '{{d.text}}',
  ...
  job.prototype.renderData = function (resp) {
    var _self = this,
      ...
      _elem = _config.elem,
      _container = _elem.next('.' + container),
      _head = _container.find('thead'),
      _dom = _container.find('tbody'),
      _list = [];
    _config.temp_data = [];
    ...
    _head.html(laytpl(_config.template_head).render(e))
  ...
  job.prototype.events = function () {
    ...
    $(document).on('click', function (e) {
      var _target = e.target, _item = _dom.find(_target), _e = _item.length > 0 ? _item.closest('tr') : undefined;
